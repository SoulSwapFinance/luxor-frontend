...
function InfoTooltipMulti({ messagesArray }) {
    ...
    return (
        <Box>
            <SvgIcon component={Info} onMouseOver={handleHover} onMouseOut={handleHover} style={{ margin: "0 5px", fontSize: 16 }} className="info-icon"></SvgIcon>
            <Popper id={id} open={open} anchorEl={anchorEl} placement="bottom" className="tooltip">
                <Paper className="info-tooltip lux-card" style={{ padding: "1.33rem" }}>
                    {messagesRender}
                </Paper>
            </Popper>
        </Box>
